...
rather than a novel molecular representation. Only one full training fold is
currently reported, and the bootstrap intervals do not include retraining
variability. Multi-fold stability assessment is prepared in the
reproducibility workflow, but I do not report a partial multi-fold estimate
because incomplete folds would give a false sense of precision. Public
retrospective data cannot establish project-specific hit-rate improvement or
cost reduction. The EGFR replay is same-source and balanced, and must not be
described as a prospective unknown-activity library. ADMET endpoints vary in
temporal generalization; weak endpoints such as CYP induction should be
treated only as low-confidence review prompts. Prospective or externally
blinded experiments are still required for stronger deployment claims. These
limitations are not side notes to the work; they define the level at which the
